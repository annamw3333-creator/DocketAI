...
def _scenario_verdict(
    ...
    *,
    ...
    """Pass/fail + severity + reasons for one scenario (may span multiple turns)."""
    ...
    # Pack-authored failure criteria become soft reasons when the run failed dims
    for fc in (scenario_meta.get("failure_criteria") or [])[:3]:
        if fail_dims and fc and fc not in reasons:
            reasons.append(str(fc))
    ...
    # Pack severity is a floor when the scenario failed
    pack_sev = str(scenario_meta.get("severity") or "").lower()
    if pack_sev in _SEVERITY_RANK and not passed:
        severity = _max_severity(severity, pack_sev)

    suggested_fixes: list[str] = []
    # Prefer pack-authored copyable fixes
    for fix in scenario_meta.get("suggested_fixes") or []:
        if fix:
...
def run_pack_on_bot(
    ...
    Multi-turn scenarios score every step; scenario scores are the aggregate.
    Optional replies map scenario_id -> assistant text (first step only) or
    scenario_id::turn_index -> assistant text. Otherwise uses simulate_reply.
    """
    ...
    for sc in shaped_pack.get("scenarios") or []:
        ...
        # Merge expectations that matter at scenario level (any-turn flags)
        ...
        for ti, step in enumerate(steps):
            ...
            if expected.get("expect_booking"):
                any_booking = True
            for kw in expected.get("must_include") or []:
                if kw not in must_include_all:
                    must_include_all.append(kw)
            # last non-empty policy_phrase wins for merged view
            if expected.get("policy_phrase"):
                merged_expected["policy_phrase"] = expected["policy_phrase"]

            reply_key_turn = f"{sc['id']}::{ti}"
            if reply_key_turn in replies:
                asst = replies[reply_key_turn]
            elif ti == 0 and sc["id"] in replies:
                asst = replies[sc["id"]]
            else:
            ...
            turns_out.append({**turn, "scores": scores, "expected": expected})

        merged_expected["expect_escalation"] = any_escalate
        merged_expected["expect_booking"] = any_booking
        if must_include_all:
            merged_expected["must_include"] = must_include_all

        scores = aggregate_scores(turn_scores)
        # First-turn user/assistant kept for Results list compatibility
        first = turns_out[0]
        last = turns_out[-1]
        scenario_meta = {
            "severity": sc.get("severity"),
            "failure_criteria": sc.get("failure_criteria") or [],
            "suggested_fixes": sc.get("suggested_fixes") or [],
            "policy_refs": sc.get("policy_refs") or [],
        }
        verdict = _scenario_verdict(scores, merged_expected, scenario_meta=scenario_meta)
        scenarios_out.append(
            {
                ...
                "policy_refs": scenario_meta["policy_refs"],
                "failure_criteria": scenario_meta["failure_criteria"],
                "lens": sc.get("lens"),
            }
        )
        # lightweight failure flags
        if any_escalate and scores.get("escalation", 0) < 50:
            failures.append({"scenario_id": sc["id"], "reason": "missed escalation"})
        if any_booking and scores.get("booking_success", 0) < 50:
